...
class App(tk.Tk):
    ...
    def refresh(self):
        ...
        details = [
            ...
            "自动发起：{}".format(data.get("resume_count", 0)),
            "自动成功：{}".format(data.get("resume_success_count", 0)),
            "你手动接回：{}".format(data.get("manual_recovery_count", 0)),
            "旧版发起历史：{}（旧版未区分最终是否成功）".format(data.get("legacy_resume_count", 0)),
